...
# <<ikarus-notes>>
- Ikarus Notes :: Unfortunatly, there is still a tweak to Ikarus
                  (needed by spells' process interface) which has not
                  yet made it into Ikarus' mainline, so you need to
                  patch the source:

: bzr checkout --lightweight http://ikarus-scheme.org/ikarus.dev ikarus
: cd ikarus
: wget -O - http://launchpadlibrarian.net/27410321/0002-t_process-creation.diff \
:     | patch -p1

                  You should build Ikarus using the following flags;
                  see [[http://bugzilla.gnome.org/show_bug.cgi?id=563464#c3][this bug on gobject-introspection]] why using
                  ~-lpthread~ is a good idea.

...
(define (thing->delim-matcher x)
  (if (procedure? x) x					; matcher proc
      (let ((re (cond ((string? x) (pregexp x))
		      ...
		      ((char? x) (pregexp (string x)))
		      ((pair? x) x) ;; assume U-regexp
		      (else (error "Illegal field-reader delimiter value" x)))))
	(lambda (s i)
	  (cond ((pregexp-match-positions re s i) =>
		 (lambda (m) (values (caar m) (cdar m))))
		(else (values #f #f)))))))
...
(define (make-field-parser-generator default-delim-matcher loop-proc)
  ...
  (lambda args
    (let-optionals args ((delim-spec default-delim-matcher)
      ...
      (let ((match-delim (thing->delim-matcher delim-spec))
...
(define default-field-matcher (thing->delim-matcher (pregexp "[^[:space:]]+")))
...
(define (field-splitter . args)
  (let-optionals args ((field-spec default-field-matcher)
    ...
    ;; Process and error-check the args
    (let ((match-field (thing->delim-matcher field-spec)))
      (receive (num-fields nfields-exact?)
	       (cond ((not num-fields) (values #f #f))
		     ((not (integer? num-fields))
...
(define default-suffix-matcher (thing->delim-matcher (pregexp "([[:space:]]+|$)")))
(define default-infix-matcher  (thing->delim-matcher (pregexp "[[:space:]]+")))
...
;; arch-tag: 0ba600ec-d7b0-4c72-a34c-b02a0e1e9f86

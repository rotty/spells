(#%require (only mzscheme
                 define-values make-struct-type struct:exn
...
(define-values (struct:condition %really-make-condition condition? %condition-ref %condition-set!)
  (make-struct-type 'condition struct:exn 1 0))

(define (really-make-condition type-field-alist)
  (%really-make-condition "SRFI-35 condition" (current-continuation-marks) type-field-alist))

(define (condition-type-field-alist c) (%condition-ref c 0))

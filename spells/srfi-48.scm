...
(define ascii-tab   (ascii->char  9))
(define unspecified (if #f #f))

;;@ Formats the arguments @3 according to the format string @2.  If @1
;; is an output port, the result is written to it. @1 also may be
;; @code{#t}, which causes the result being written to
;; @code{(current-output-port)}, or @code{#f} to have the result
...
(define (format output-port format-string . args)
  ((cond ((output-port? output-port) (lambda (proc)
                                       (proc output-port)))
         ((eq? output-port #t) (lambda (proc)
                                 (proc (current-output-port))))
         ((eq? output-port #f) call-with-string-output-port) 
         (else (error
